 ...
 *         (no actual async ops are happening)
 */
const decodeFields = function decodeFields (payload, box, timestamp) {
  return Promise.resolve().then(function () {
    // should never be thrown, as we find a box by it's ttn config
    if (!payload.length) {
      throw new Error('payload may not be empty');
    ...
    // decode buffer using bufferTransformer
    //const measurements = bufferToMeasurements(buffer, bufferTransformer);
    const mapping = box.integrations.ttn.decodeOptions, sensorMatchings = [];


    for (const channel of mappings) {
        const match = {};
        if (channel.sensor_id) {
          match['_id'] = [channel.sensor_id];
        }
        if (channel.sensor_title) {
          match['title'] = [channel.sensor_title];
        }
        if (channel.sensor_type) {
          match['sensorType'] = [channel.sensor_type];
        }
        if (channel.sensor_unit) {
          match['unit'] = [channel.sensor_unit];
        }
        channel['_id'] = findSensorIds(box.sensors, [match])[0]
    }
    const measurements = [];
    for (const m of payload) {
        lpp_id = m.split("_").pop()
        channel = mappings.find(c => {
            return c.channel == lpp_id
        })
        measurements.push({
          sensor_id: channel._id,
          value: payload[m]
        });
    }
    // if a timestamp is provided, set it for all the measurements
...
const decodeRequest = function decodeRequest (req) {
  ...
  if (req.box.integrations.ttn.profile === 'cayennelpp') {
    return req.body.payload_fields
      ? decodeFields(req.body.payload_fields)
      : Promise.reject('no payload for profile `cayennelpp` provided');
  }
